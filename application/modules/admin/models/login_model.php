...
public function get_login($username, $password){
		...
		$this->db->where(array('systemstatus'=>'Enable','deletionstatus'=>'No'));		
		return $this->db->get()->row();	
	}
function check_school_status_by($where_array){
	   $this->db->select('*');
	   $this->db->from('schools');
       $this->db->where($where_array);
return $this->db->get()->row();	
}
/*function check_academicsession_status_by($where_array){
	   $this->db->select('*');
	   $this->db->from('academicsession');
	   $this->db->where('acse_startyear<=','NOW()',FALSE);
	   $this->db->where('acse_endyear>=','NOW()',FALSE);
	   $this->db->where('acse_startdate<=','NOW()',FALSE);
	   $this->db->where('acse_enddate>=','NOW()',FALSE);
       $this->db->where($where_array);
	   $this->db->where(array('acse_systemstatus'=>'Enable','acse_deletionstatus'=>'No'));	
return $this->db->get()->row();	
}*/
function update_profile_byuserid($adminid, $update_array){ 
         $this->db->update('admin', $update_array, array('adminid' => $adminid)); 
  return $this->db->affected_rows();   	      
}	
function update_password_byadminid($adminid, $correntpassword, $newpassword){   
		...
		$this->db->set('firstlogin','No');
		$this->db->update('admin');
		return $this->db->affected_rows();	 
	      
}	
function verify_username_byemail($where_array){
	   $this->db->select('*');
	   $this->db->from('admin');
       $this->db->where($where_array);
return $this->db->get()->row();	
}	
function update_username_byemail($where_array, $update_array){
	   $this->db->set('emailcount','emailcount+1',FALSE);
	   $this->db->update('admin', $update_array, $where_array); 
       return $this->db->affected_rows();
}		

function set_new_password_by($where_array, $update_array, $new_password){
...
///////////////////////////////////////Credit Sale notifications//////////////////
 function get_credit_sales_count_alert(){
	   $this->db->select('*');
	   $this->db->from('admin');
      // $this->db->where($where_array);
return $this->db->get();	
   }
function ajax_get_credit_sales_notification(){
     $this->db->select('*');
	   $this->db->from('admin');
      // $this->db->where($where_array);
return $this->db->get();	
  } 
function get_recently_sales_count_alert(){
 $this->db->select('*');
	   $this->db->from('admin');
      // $this->db->where($where_array);
return $this->db->get();	
  }
function ajax_get_recently_sales_notification(){
     $this->db->select('*');
	   $this->db->from('admin');
      // $this->db->where($where_array);
return $this->db->get();	
